...
const GovernmentLayout = () => {
    ...
    return (
        <div className="flex min-h-screen bg-[#f8fafc] overflow-hidden">
            {/* Mobile Header */}
            <div className="md:hidden fixed top-0 w-full bg-[#0d1b2a] text-white p-4 flex items-center justify-between z-[60] shadow-md">
                <span className="font-black text-sm tracking-widest uppercase text-emerald-400">Gov Dashboard</span>
            ...
            <div className={`fixed inset-y-0 left-0 transform ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 ease-in-out z-[50] md:relative md:flex h-screen`}>
                <GovernmentSidebar onClose={() => setIsMobileOpen(false)} />
            </div>

            {/* Main Content Area */}
            <main className="flex-1 w-full overflow-y-auto h-screen p-4 pt-20 md:pt-8 md:p-8 custom-scrollbar">
