...
const GovernmentSidebar = () => {
    ...
    return (
        <aside className="w-64 bg-[#0d1b2a] text-white flex flex-col h-screen fixed left-0 top-0 overflow-y-auto">
            {/* Header / Logo */}
                ...
                <div className="bg-[#1b263b] rounded-xl p-4 flex items-center gap-3 cursor-pointer hover:bg-white/10 transition-colors">
                    <div className="w-10 h-10 rounded-full bg-slate-700 overflow-hidden flex items-center justify-center border border-slate-600">
                        {currentUser?.photoURL ? (
                            ...
                            <span className="font-bold text-lg">{currentUser?.displayName?.[0] || 'M'}</span>
                        )}
                    </div>
                    <div className="flex-1 overflow-hidden">
                        <p className="text-sm font-bold truncate">{currentUser?.displayName || 'M. Ramesh'}</p>
